...
class Electra_aoa(nn.Module):
    ...
    def forward(self, input, **kwargs):
        ...
        ##
        #modified here to accept custom mask selector
        ##
        ...
        if self.mask_selector is not None:
            ...
            if attention_mask is None:
                attention_mask = torch.ones_like(input, dtype=torch.bool)
            else:
                attention_mask = attention_mask.bool()
            
            #getting the eligible mask positions
            eligible_mask = self.mask_selector(input, attention_mask)
            mask = get_mask_subset_with_prob(eligible_mask, self.mask_prob)

        else:
            no_mask = mask_with_tokens(input, self.mask_ignore_token_ids)
            mask = get_mask_subset_with_prob(~no_mask, self.mask_prob)
        
        # this is a safety check in case no tokens were masked, which can happen in curriculum learning settings
        # especially at the beginning of training, when the masking probability is very low, and many tokens are excluded from masking
        # we need to maintain the MLM task, so we fallback to standard BERT-style masking on non-special positions
        # especially important when using curriculum learning with very low initial masking probabilities
        # modified here to accept custom mask selector, and maintain adversarial masking so we do not have issues in the training loop for the gradient flow
        if not mask.any():
            # fallback: standard BERT-style masking on non-special positions
            no_mask = mask_with_tokens(input, self.mask_ignore_token_ids)
            fallback_space = (~no_mask) & attention_mask.bool()
            mask = get_mask_subset_with_prob(fallback_space, max(self.mask_prob, 0.10))
